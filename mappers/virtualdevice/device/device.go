...
func dataHandler(ctx context.Context, dev *driver.CustomizedDev) {
	for _, twin := range dev.Instance.Twins {
		...
		go twinData.Run(ctx)
		// handle push method
		//klog.V(1).Infof("twin.PVisitor.PushMethod = %v", twin.PVisitor.PushMethod)
		//klog.V(1).Infof("MethodConfig = %v", twin.PVisitor.PushMethod.MethodConfig)
		//klog.V(1).Infof("MethodName = %v", twin.PVisitor.PushMethod.MethodName)
		//testconfig := make(map[string]interface{})
		//err = json.Unmarshal(twin.PVisitor.PushMethod.MethodConfig, &testconfig)
		//if err == nil {
		//	klog.V(1).Infof("testconfig = %v", testconfig)
		//} else {
		//	klog.Error(err)
		//}
...
func setVisitor(visitorConfig *driver.VisitorConfig, twin *common.Twin, dev *driver.CustomizedDev) error {
	klog.V(1).Infof("twin.PVisitor.PProperty = %v", twin.PVisitor.PProperty)
	klog.V(1).Infof("twin.PVisitor.PProperty.AccessMode = %s", twin.PVisitor.PProperty.AccessMode)
